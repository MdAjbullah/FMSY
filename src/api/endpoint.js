...
//       },
//     });
//     console.log("Policies retrieved successfully:", response.data);
//     return response.data;
//   } catch (error) {
...
export const getLoanReminders = async () => {
  try {
    ...
    const response = await axios.get(`${API_URL}loans/reminders/`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
export const fetchLoans = async () => {
  try {
    const response = await axios.get(`${API_URL}loansda/`);
    return response.data; // return loan data
  } catch (error) {
    throw new Error("Error fetching loan data: " + error.message);
  }
};
